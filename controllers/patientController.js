    ...
    try {
        ...
        if (role === 'doctor') {
            sql = `
                ...
                WHERE p.is_active = 1 
                AND (
                    p.doctor_id = (SELECT id FROM doctors WHERE user_id = ? LIMIT 1)
                    OR 
                    (p.doctor_id IS NULL AND p.department = (SELECT specialization FROM doctors WHERE user_id = ? LIMIT 1))
                )
                ORDER BY p.id DESC
            `;
            ...
            sql = `
                SELECT 
                    id, name, pid, email, age, gender, doctor_id, is_active, is_visited, phone, department, user_id,
...
exports.getDoctorStats = asyncHandler(async (req, res) => {
    ...
    const [doc] = await db.query("SELECT id, specialization FROM doctors WHERE user_id = ? LIMIT 1", [userId]);
