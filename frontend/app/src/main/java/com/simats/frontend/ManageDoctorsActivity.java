...
public class ManageDoctorsActivity extends AppCompatActivity {
    ...
    private void loadDoctors() {
        ...
        apiInterface.getDoctors().enqueue(new retrofit2.Callback<List<com.google.gson.JsonObject>>() {
            @Override
            public void onResponse(retrofit2.Call<List<com.google.gson.JsonObject>> call, retrofit2.Response<List<com.google.gson.JsonObject>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    for (com.google.gson.JsonObject json : response.body()) {
                        ...
                        String spec = json.has("specialization") && !json.get("specialization").isJsonNull() ? json.get("specialization").getAsString() : "General";
                        ...
                        String title = json.has("professional_title") && !json.get("professional_title").isJsonNull() ? json.get("professional_title").getAsString() : "Staff";
                        
                        doctorList.add(new Doctor(id, name, spec, "ACTIVE", title, "10 Years", phone, email));
                    }
                    adapter = new DoctorAdapter(ManageDoctorsActivity.this, doctorList);
                    binding.rvDoctors.setAdapter(adapter);
                } else {
                    Toast.makeText(ManageDoctorsActivity.this, "Failed to load doctors", Toast.LENGTH_SHORT).show();
                }
            }

            @Override
            public void onFailure(retrofit2.Call<List<com.google.gson.JsonObject>> call, Throwable t) {
                Toast.makeText(ManageDoctorsActivity.this, "Network error: " + t.getMessage(), Toast.LENGTH_SHORT).show();
            }
        });
    }
}
