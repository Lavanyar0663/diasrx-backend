...
exports.getPrescriptionsByPatient = (req, res, next) => {
    ...
    let sql = `
        SELECT p.*, u.name as doctor_name, u.department as doctor_department, DATE_FORMAT(p.created_at, '%d/%m/%Y, %H:%i') as formatted_date
        FROM prescription p
...
exports.getPendingPrescriptions = asyncHandler(async (req, res, next) => {
    const sql = `
        SELECT p.*, u.name as doctor_name, u.department as doctor_department, pat.name as patient_name, pat.id as patient_display_id,
...
exports.getPrescriptionById = (req, res, next) => {
    ...
    const prescSql = `
        SELECT p.*, u.name as doctor_name, u.role as doctor_role, pat.name as patient_name, pat.id as patient_display_id, pat.age, pat.gender
        FROM prescription p
        JOIN users u ON p.doctor_id = u.id
        JOIN patients pat ON p.patient_id = pat.id
        WHERE p.id = ?`;
